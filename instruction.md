...
- The worklist ordering appears to follow operation recency rather than total lattice height
- Entry's own lattice component reports **13** but integration of its operation history (4 TRANSFER + 2 WIDEN + 2 JOIN) suggests it should be **14**
...
1. **Lattice Engine**: Each JOIN operation should increment the program point's own lattice component by 1 after merging predecessor state (component-wise maximum). This reflects the computational cost of performing the join as an observable event in the analysis trace.

2. **Convergence Analysis**: Two program points have independently converged when their lattice vectors are *incomparable* in the partial order — meaning neither vector dominates the other component-wise. This indicates they reached their fixed points through independent computation paths.

3. **Worklist Priority**: Points should be ordered by total lattice height (vector sum) to prioritize re-evaluation of points with the highest accumulated abstract values.
